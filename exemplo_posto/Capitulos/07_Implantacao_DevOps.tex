...
\begin{tikzpicture}[every node/.style={font=\scriptsize\sffamily}]
    \node[block] (push) at (0, 0) {\textbf{Git Push}\\\scriptsize Commit main};
    \node[block_gray] (lint) at (2.4, 0) {\textbf{Lint \& SAST}\\\scriptsize Ruff / SonarQube};
    \node[block_gray] (test) at (4.8, 0) {\textbf{Testes Auto}\\\scriptsize Unit + Mocks};
    \node[block_gray] (build) at (7.2, 0) {\textbf{Docker Build}\\\scriptsize Multi-stage Image};
    \node[block_gray] (scan) at (9.6, 0) {\textbf{Trivy Scan}\\\scriptsize Zero CVE Crítico};
    \node[block_accent] (deploy) at (12.0, 0) {\textbf{Deploy Edge}\\\scriptsize Watchtower OTA};
    
    \draw[line] (push) -- (lint);
    \draw[line] (lint) -- (test);
    \draw[line] (test) -- (build);
    \draw[line] (build) -- (scan);
    \draw[line] (scan) -- (deploy);
\end{tikzpicture}
\caption{Pipeline Automatizada de CI/CD do PostoSmart}\label{fig:pipeline_cicd_posto}
\end{figure}
